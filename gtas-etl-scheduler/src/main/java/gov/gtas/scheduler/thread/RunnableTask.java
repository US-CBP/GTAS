...
public class RunnableTask implements Runnable {
	...
	public synchronized void executeOnCommandLine(String command) {
		int exitValue = -1;
		try {
			String etlInfo = "* * * Launching " + appConfig.getEtlName() + " job from GTAS ETL SCHEDULER * * *";
			log.info(etlInfo);
			...
			isProcessing = false;
			log.info("***  END OF GTAS ETL SCHEDULER .....EXIT VALUE = " + exitValue);
			process.destroy();

		} catch (Exception e) {
			String errorInfo = "An error has occurred when launching  " + appConfig.getEtlName() + " from GTAS ETL Scheduler.";
			log.error(errorInfo);
		}
