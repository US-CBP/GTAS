 ...
 *
 * @author gbays
 * This is a convenience utility for micro benchmarking code.
 * Use unique values for the name param; it does not matter what you put for name, it just
 * allows the retrieval of the TimeObject for the benchmark.
 * If the name is not unique for a run, then a negative time (-99999) will be returned.
 * The Logger level is set to Warn here to make the messages more visible in the output.
 */
public class Bench 
{
    ...
    {
       List<TimeObject> reducedOrderedList = 
               timeObjectMap.values().stream()
               .filter(t -> t.isIsEndObject())
               .sorted((t1,t2) -> t2.getElapsedTime().compareTo(t1.getElapsedTime()))
