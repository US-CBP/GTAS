...
public class JsonValidationUtils {
	...
	public static void validateMetaData(final MetaData metaData, final boolean checkStartDate) {
		if (metaData == null) {
			throw new CommonServiceException(RuleErrorConstants.NO_META_ERROR_CODE,
					RuleErrorConstants.NO_META_ERROR_MESSAGE);
		}
		final String title = metaData.getTitle();
		if (StringUtils.isEmpty(title)) {
			throw new CommonServiceException(RuleErrorConstants.NO_TITLE_ERROR_CODE,
		...
		// If the start date is somehow before the current date, just send message to
	 ...
	 *            if true checks that the start date is greater than or equal to
	 *            today.
	 */
	private static String validateDates(final Date startDate, final Date endDate, final boolean checkStartDate) {
		...
		if (startDate == null) {
			throw new CommonServiceException(RuleErrorConstants.INVALID_START_DATE_ERROR_CODE,
		...
		if (DateCalendarUtils.dateRoundedLess(endDate, startDate, Calendar.DATE)) {
			throw new CommonServiceException(RuleErrorConstants.END_LESS_START_DATE_ERROR_CODE,
