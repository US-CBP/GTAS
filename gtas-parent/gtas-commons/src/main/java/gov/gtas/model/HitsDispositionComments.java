...
public class HitsDispositionComments extends BaseEntityAudit implements Serializable {
	...
	@JsonIgnore
	@ManyToMany(targetEntity = Attachment.class, cascade = { CascadeType.MERGE, CascadeType.PERSIST,
			CascadeType.REFRESH }, fetch = FetchType.EAGER)
	@JoinTable(name = "hits_disposition_comments_attachment", joinColumns = @JoinColumn(name = "hits_disp_comment_id"), inverseJoinColumns = @JoinColumn(name = "attachment_id"))
	...
	public boolean equals(Object o) {
		...
		return new EqualsBuilder().appendSuper(super.equals(o)).append(hitDisp, that.hitDisp).append(hitId, that.hitId)
				.append(id, that.id).isEquals();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder(17, 37).appendSuper(super.hashCode()).append(hitDisp).append(hitId).append(id)
				.toHashCode();
	}
}
