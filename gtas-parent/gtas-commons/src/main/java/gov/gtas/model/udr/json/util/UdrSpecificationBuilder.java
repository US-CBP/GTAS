...
public class UdrSpecificationBuilder {
	...
	public UdrSpecificationBuilder addMeta(String title, String descr, Date startDate, Date endDate, boolean enabled,
			String author) {
	...
	public UdrSpecificationBuilder addTerm(EntityEnum entity, String attr, TypeEnum type, CriteriaOperatorEnum op,
			String[] val) {
		queryObjectStack.peek().getRules()
				.add(new QueryTerm(entity.getEntityName(), attr, type.toString(), op.toString(), val));
		return this;
	}
	...
	/**
	 * Creates a sample UDR specification JSON object. (This is used for testing.)
	 * 
	 * @param userId
	 * @param title
	 * @param description
	...
	public static UdrSpecification createSampleSpec(String userId, String title, String description) {
		final UdrSpecificationBuilder bldr = new UdrSpecificationBuilder(null, QueryConditionEnum.OR);
		bldr.addTerm(EntityEnum.PASSENGER, PassengerMapping.DOB.getFieldName(), TypeEnum.DATE,
				CriteriaOperatorEnum.EQUAL, new String[] { DateCalendarUtils.formatJsonDate(new Date()) });
		bldr.addTerm(EntityEnum.PASSENGER, PassengerMapping.LAST_NAME.getFieldName(), TypeEnum.STRING,
				CriteriaOperatorEnum.EQUAL, new String[] { "Jones" });
		bldr.addNestedQueryObject(QueryConditionEnum.AND);
		bldr.addTerm(EntityEnum.PASSENGER, PassengerMapping.EMBARKATION.getFieldName(), TypeEnum.STRING,
				CriteriaOperatorEnum.IN, new String[] { "DBY", "PKY", "FLT" });
		bldr.addTerm(EntityEnum.PASSENGER, PassengerMapping.DEBARKATION.getFieldName(), TypeEnum.STRING,
	...
	public static UdrSpecification createSampleSpec2(String userId, String title, String description) {
		final UdrSpecificationBuilder bldr = new UdrSpecificationBuilder(null, QueryConditionEnum.AND);
		bldr.addTerm(EntityEnum.PASSENGER, PassengerMapping.DOB.getFieldName(), TypeEnum.DATE,
				CriteriaOperatorEnum.EQUAL, new String[] { DateCalendarUtils.formatJsonDate(new Date()) });
		bldr.addTerm(EntityEnum.PASSENGER, PassengerMapping.LAST_NAME.getFieldName(), TypeEnum.STRING,
				CriteriaOperatorEnum.EQUAL, new String[] { "Jones" });
	...
	public static UdrSpecification createSampleSpec3(String userId, String title, String description) {
		final UdrSpecificationBuilder bldr = new UdrSpecificationBuilder(null, QueryConditionEnum.OR);
		bldr.addNestedQueryObject(QueryConditionEnum.AND);
		bldr.addTerm(EntityEnum.PASSENGER, PassengerMapping.DOB.getFieldName(), TypeEnum.DATE,
				CriteriaOperatorEnum.EQUAL, new String[] { DateCalendarUtils.formatJsonDate(new Date()) });
