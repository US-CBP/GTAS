...
public class WatchlistValidationAdapter {
	public static void validateWatchlistSpec(WatchlistSpec wljson) {
		BeanPropertyBindingResult errors = new BeanPropertyBindingResult(wljson, "watch list");
		...
		for (WatchlistItemSpec itm : wljson.getWatchlistItems()) {
			...
			if (itm.getTerms() == null && action != WatchlistEditEnum.D) {
				throw ErrorHandlerFactory.getErrorHandler().createException(
						CommonErrorConstants.NULL_ARGUMENT_ERROR_CODE, "Watch list rule items",
				...
				for (WatchlistTerm trm : itm.getTerms()) {
					QueryTerm t = new QueryTerm(wljson.getEntity(), trm.getField(), trm.getType(),
							CriteriaOperatorEnum.EQUAL.toString(), new String[] { trm.getValue() });
					terms.add(t);
				}
	...
	private static WatchlistEditEnum validateAction(WatchlistItemSpec itm, Errors errors) {
		WatchlistEditEnum action = null;
		try {
			action = WatchlistEditEnum.getEditEnumForOperationName(itm.getAction());
			switch (action) {
			case C:
				if (itm.getId() != null) {
					errors.rejectValue("id", CommonErrorConstants.INVALID_ARGUMENT_ERROR_CODE,
							"Cannot specify id for create action - " + itm.getId());
				}
				break;
			case U:
			case D:
				if (itm.getId() == null) {
					errors.rejectValue("id", CommonErrorConstants.NULL_ARGUMENT_ERROR_CODE,
							"No id specified for update or delete action");
				}
				break;
			}
		} catch (IllegalArgumentException iae) {
			errors.rejectValue("action", CommonErrorConstants.INVALID_ARGUMENT_ERROR_CODE,
