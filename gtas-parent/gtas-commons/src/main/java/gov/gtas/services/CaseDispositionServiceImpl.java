...
public class CaseDispositionServiceImpl implements CaseDispositionService {
	...
	public Case create(Long flight_id, Long pax_id, String paxName, String paxType, String nationality, Date dob,
		...
		if (caseMap == null || caseMap.isEmpty() || !caseMap.containsKey(pax_id)) {
			 ...
			 * data structure returns list as many results are possible. Only 1 result is
			 * expected. More are possible under the condition of a user changing a closed
			 * case to open status AFTER getting a new case generated (or after generating a
			 * manual case).
			 *
			 */
	...
	public Case addCaseComments(Long caseId, Long hit_id, String caseComments, String status, String validHit,
		...
		try {
			aCase = caseDispositionRepository.findOne(caseId); // getCaseByFlightIdAndPaxId(flight_id, pax_id,
	...
	public List<Case> registerAndSaveNewCaseFromFuzzyMatching(Long flight_id, Long pax_id, String paxName,
		...
		_tempCaseList.add(create(flight_id, pax_id, paxName, paxType, nationality, dob, document, hitDesc, _tempHitIds,
				caseMap, flightMap, passengerMap, ruleCatMap));
	...
	public List<OneDayLookoutVo> getOneDayLookoutByDateAndAirport(Date date, String airport) {
		...
		List<Case> oneDayLookoutResult = caseDispositionRepository.findOneDayLookoutByDateAndAirport(startDate, endDate,
				airport);
