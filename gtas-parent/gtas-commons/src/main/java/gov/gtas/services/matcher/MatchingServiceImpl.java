...
public class MatchingServiceImpl implements MatchingService {
    ...
    private PaxWatchlistLinkVo convertToVo(float percentMatch, Date lastRunTimestamp, int verifiedStatus,
        ...
        try {
            ...
            return new PaxWatchlistLinkVo(percentMatch, lastRunTimestamp, verifiedStatus, passenger.getId(), item.getId(), firstName, lastName, dob);
        } catch (IOException ioe) {
            logger.error("Converting watchlist item hit to vo failed!"
                    + ioe.getMessage());
    ...
    public List<PaxWatchlistLinkVo> findByPassengerId(Long id) {
        List<PaxWatchlistLinkVo> pwlList = new ArrayList<>();
