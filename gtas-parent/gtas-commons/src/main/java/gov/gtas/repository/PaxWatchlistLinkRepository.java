...
public interface PaxWatchlistLinkRepository extends JpaRepository<PaxWatchlistLink, Long> {
	...
	@Query("SELECT p FROM PaxWatchlistLink p " + "left join fetch p.watchlistItem wlItem "
			+ "left join fetch wlItem.watchlistCategory " + "WHERE p.passenger.id =:paxId")
	...
	public void savePaxWatchlistLink(@Param("lastDate") Date lastDate, @Param("percentMatch") float percentMatch,
			@Param("verifiedStatus") int verifiedStatus, @Param("passengerId") Long passengerId,
