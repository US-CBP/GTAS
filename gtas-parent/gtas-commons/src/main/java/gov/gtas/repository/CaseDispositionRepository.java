...
public interface CaseDispositionRepository extends JpaRepository<Case, Long>, CaseDispositionRepositoryCustom {
	...
	@Query("SELECT c FROM Case c WHERE c.flightId = (:flightId) " + "AND c.paxId = (:paxId) AND c.status in :status")
	public List<Case> getCaseByFlightIdAndPaxId(@Param("flightId") Long flightId, @Param("paxId") Long paxId,
			@Param("status") List<String> status);

	@Query("SELECT c FROM Case c WHERE " + " c.paxId in (:paxIds)")
	public List<Case> getCaseByPaxId(@Param("paxIds") List<Long> paxIds);
	...
	@Query("SELECT c " + "FROM Case c " + "Where c.flightId in :flightIds and c.status = 'NEW'")
	...
	// @Modifying
	// @Transactional
	// @Query("update HitsDisposition set hitsDispositions = :hitsDispositionSet
	// where id = :id")
	// public Integer updateDispCommentsForHitsDisposition(Long id,
	// Set<HitsDisposition> hitsDispositionSet);
	...
	public List<Case> findOneDayLookoutByDateAndAirport(@Param("startDate") Date startDate,
			@Param("endDate") Date endDate, @Param("airport") String airport);
	...
	public void updateEncounteredStatus(@Param("caseId") Long caseId,
			@Param("encStatus") EncounteredStatusEnum encStatus);
