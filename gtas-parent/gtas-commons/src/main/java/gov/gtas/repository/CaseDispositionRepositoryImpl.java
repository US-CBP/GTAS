...
public class CaseDispositionRepositoryImpl implements CaseDispositionRepositoryCustom {
	...
	private static final Logger logger = LoggerFactory.getLogger(CaseDispositionRepositoryImpl.class);
	...
	public Pair<Long, List<Case>> findByCriteria(CaseRequestDto dto) {
		...
		querySB.append(
				"SELECT c, CASE WHEN c.flight.direction= 'I' THEN c.flightETADate WHEN c.flight.direction= 'O' THEN c.flightETDDate ELSE c.flightETDDate END AS countdown ");
		...
		if (dto.getDisplayStatusCheckBoxes() != null
				&& !dto.getDisplayStatusCheckBoxes().namesOfCheckedBoxes().isEmpty()) {
		...
		String finalQuery = querySB.toString().endsWith(",")
				? querySB.toString().substring(0, querySB.toString().length() - 1)
				: querySB.toString();
	...
	private void addCaseDispositionSql(List<String> namesOfCheckedBoxes, StringBuilder querySB,
			StringBuilder countQuerySB) {
		querySB.append(" AND ( ");
		countQuerySB.append(" AND ( ");
	...
	private List<String> addCriteria(CaseRequestDto dto) {
		...
		if (dto.getEtaStart() != null && dto.getEtaEnd() != null) {
			criteria.add(
					" (c.flightETADate BETWEEN :etaStart AND :etaEnd OR  c.flightETDDate BETWEEN :etaStart AND :etaEnd) ");
		}

		if (dto.getUserLocation() != null && !dto.getUserLocation().isEmpty()) {
			criteria.add(
					"( ( UPPER(c.flight.direction)='I' AND c.flight.destination = :userLocation) OR ( UPPER(c.flight.direction)='O' AND c.flight.origin = :userLocation) )");

		}

		if (dto.getWithTimeLeft() != null && dto.getWithTimeLeft()) {
			criteria.add(
					" ( (c.flightETADate > :now and c.flight.direction = 'I') OR (c.flightETDDate > :now and c.flight.direction = 'O') ) ");
		}
	...
	private void setQueryParameters(Query query, Query countQuery, CaseRequestDto dto) {
		...
		if (dto.getEtaStart() != null && dto.getEtaEnd() != null) {
			...
			calStart.set(calStart.get(Calendar.YEAR), calStart.get(Calendar.MONTH), calStart.get(Calendar.DAY_OF_MONTH),
					0, 0, 0);
