...
public class ErrorUtils {
	/**
	 * Searches the exception cause chain to determine if any cause is of the given
	 * type.
	 * 
	 * @param exception
	 ...
	 * @return true if the exception type is found.
	 */
	public static boolean isConstraintViolationException(Throwable exception, String constraintName) {
		boolean ret = false;
		Throwable cause = exception;
		while (cause != null) {
			if (cause instanceof ConstraintViolationException
					&& ((ConstraintViolationException) cause).getConstraintName().equals(constraintName)) {
				ret = true;
				break;
			} else {
				cause = cause.getCause();
	...
	public static BasicErrorDetailInfo createErrorDetails(String errorCode, Exception exception,
