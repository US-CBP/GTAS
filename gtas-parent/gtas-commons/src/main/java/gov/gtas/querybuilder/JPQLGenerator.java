...
//					}
     ...
     * @param queryType           indicates whether the user is querying against the flight or
     *                            passenger data
     * @param joinEntities        contains the list of entities that will later be used to generate
     *                            the join condition
            ...
            if (entityEnum == EntityEnum.HITS) {
                if (field.equalsIgnoreCase(Constants.IS_RULE_HIT)
                    ...
                    if (queryType == EntityEnum.FLIGHT) {
                        ...
                        where.append(" ").append(EntityEnum.HITS.getAlias()).append(Constants.HITS_FLIGHT_REF).append(" ").append(Constants.FROM).append(" ").append(EntityEnum.HITS.getEntityName()).append(" ").append(EntityEnum.HITS.getAlias()).append(" ").append(Constants.WHERE).append(" ").append(EntityEnum.HITS.getAlias()).append(Constants.HITS_FLIGHT_REF).append(" = ").append(EntityEnum.FLIGHT.getAlias()).append(Constants.ID).append(" ");

                        if (field.equalsIgnoreCase(Constants.IS_RULE_HIT)) {
                            where.append(Constants.AND + " ").append(EntityEnum.HITS.getAlias()).append(".").append(Constants.RULE_HIT_TYPE).append(")");
                        } else {
                            where.append(Constants.AND + " (").append(EntityEnum.HITS.getAlias()).append(".").append(Constants.PASSENGER_HIT_TYPE).append(" ").append(Constants.OR).append(" ").append(EntityEnum.HITS.getAlias()).append(".").append(Constants.DOCUMENT_HIT_TYPE).append("))");
                        }
                    } else if (queryType == EntityEnum.PASSENGER) {
                        if (value.equals("1")) {
                            where.append(Constants.EXISTS_HITS_PREFIX);
                        ...
                        where.append(" ").append(EntityEnum.HITS.getAlias()).append(Constants.HITS_PASSENGER_REF).append(" ").append(Constants.FROM).append(" ").append(EntityEnum.HITS.getEntityName()).append(" ").append(EntityEnum.HITS.getAlias()).append(" ").append(Constants.WHERE).append(" ").append(EntityEnum.HITS.getAlias()).append(Constants.HITS_PASSENGER_REF).append(" = ").append(EntityEnum.PASSENGER.getAlias()).append(Constants.ID).append(" ");
                    ...
                    if (queryType == EntityEnum.FLIGHT) {
                        where.append("(").append(EntityEnum.FLIGHT.getAlias()).append(Constants.ID).append(" = ").append(EntityEnum.HITS.getAlias()).append(Constants.HITS_FLIGHT_REF).append(" ").append(Constants.AND).append(" ").append(EntityEnum.HITS.getAlias()).append(Constants.ID).append(" ").append(opEnum.getOperator()).append(" ?").append(positionalParameter).append(")");
                    } else if (queryType == EntityEnum.PASSENGER) {
                        where.append("(").append(EntityEnum.PASSENGER.getAlias()).append(Constants.ID).append(" = ").append(EntityEnum.HITS.getAlias()).append(Constants.HITS_PASSENGER_REF).append(" ").append(Constants.AND).append(" ").append(EntityEnum.HITS.getAlias()).append(Constants.ID).append(" ").append(opEnum.getOperator()).append(" ?").append(positionalParameter).append(")");
                    }
                }
                ...
                if (OperatorEnum.IS_EMPTY.toString().equalsIgnoreCase(operator)
                    ...
                    where.append(EntityEnum.BOOKING_DETAIL.getAlias()).append(".").append(field).append(" ").append(opEnum.getOperator());
                    ...
                    if (values != null && values.size() == 2) {
                        ...
                        where.append(EntityEnum.BOOKING_DETAIL.getAlias()).append(".").append(field).append(" ").append(opEnum.getOperator()).append(" ?").append(positionalParameter);
                    ...
                    where.append(EntityEnum.BOOKING_DETAIL.getAlias()).append(".").append(field).append(" ").append(opEnum.getOperator()).append(" (?").append(positionalParameter).append(")");
                } else {
                    positionalParameter.increment(); // parameter position in the query
                ...
                if (OperatorEnum.IS_EMPTY.toString().equalsIgnoreCase(operator)
                    ...
                    if (field.equalsIgnoreCase(Constants.SEAT)) {
                        ...
                        where.append(entityEnum.getAlias()).append(".").append(field).append(" ").append(opEnum.getOperator());
                    }
                } else if (OperatorEnum.BETWEEN.toString().equalsIgnoreCase(operator)
                        || OperatorEnum.NOT_BETWEEN.toString().equalsIgnoreCase(operator)) {
                    List<String> values = null;
