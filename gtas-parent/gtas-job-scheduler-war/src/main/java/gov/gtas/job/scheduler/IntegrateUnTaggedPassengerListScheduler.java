...
public class IntegrateUnTaggedPassengerListScheduler {
	...
	private static final Logger logger = LoggerFactory.getLogger(IntegrateUnTaggedPassengerListScheduler.class);
	...
	// @Scheduled(fixedDelayString = "${cleanup.fixedDelay.in.milliseconds}",
	// initialDelayString = "${cleanup.initialDelay.in.milliseconds}")
	public void jobScheduling() throws IOException {
		...
		for (Passenger _tempPax : paxList) {
			/*
			 * if(_tempPax.getPaxIdTag() == null){ paxListWithNullIdTags.add(_tempPax);
			 * }else{ paxListNotNull.add(_tempPax); }
			 */
		}
		...
		try {
			for (Passenger _tempPaxWithIdTag : paxListNotNull) {
				...
				for (Passenger _tempPaxWithNoIdTag : paxListWithNullIdTags) {
					if ((_tempPaxWithIdTag.getPassengerDetails().getFirstName()
							.equalsIgnoreCase(_tempPaxWithNoIdTag.getPassengerDetails().getFirstName()))
							&& (_tempPaxWithIdTag.getPassengerDetails().getLastName()
									.equalsIgnoreCase(_tempPaxWithNoIdTag.getPassengerDetails().getLastName()))
							&& (_tempPaxWithIdTag.getPassengerDetails().getDob()
									.equals(_tempPaxWithNoIdTag.getPassengerDetails().getDob()))
							// &&
							// (_tempPaxWithIdTag.getNationality().equalsIgnoreCase(_tempPaxWithNoIdTag.getNationality()))
							&& (_tempPaxWithIdTag.getPassengerDetails().getGender()
									.equalsIgnoreCase(_tempPaxWithNoIdTag.getPassengerDetails().getGender()))) {
						// match found
		...
		for (Passenger _checkedListOfNullIdTagPassenger : paxListWithNullIdTags) {
			...
			/*
			 * if(_checkedListOfNullIdTagPassenger.getPaxIdTag()==null){ // Passenger found,
			 * apply IdTag to the new record _tempPaxIDTag.setIdTag(generateRandomIDTag(9));
	...
	// */
	// private String getHashForPassenger(String firstName, String lastName, String
	// gender, String DOB, String ctz_country) throws NoSuchAlgorithmException,
	// UnsupportedEncodingException{
	// return makeSHA1Hash(String.join("", Arrays.asList(firstName.toUpperCase(),
	// lastName.toUpperCase(), gender.toUpperCase(), DOB,
	// ctz_country.toUpperCase())));
	// }
	//
	// /**
	...
	// * @throws UnsupportedEncodingException
	// */
	// private String getHashForPassenger(Passenger pax) throws
	// NoSuchAlgorithmException, UnsupportedEncodingException{
	// return makeSHA1Hash(String.join("",
	// Arrays.asList(pax.getFirstName().toUpperCase(),
	// pax.getLastName().toUpperCase(),
	// pax.getGender().toUpperCase(), pax.getDob().toString(),
	// pax.getNationality().toUpperCase())));
	// }
