...
public class QueryBuilderServiceIT {
     ...
     * @Test(expected = QueryAlreadyExistsException.class)
     * 
     * @Transactional public void testEditQueryAlreadyExists() throws
     * QueryAlreadyExistsException, InvalidQueryException,
     * QueryDoesNotExistException { UserQueryRequest request = new
     * UserQueryRequest();
     * 
     ...
     * request.setDescription(DESCRIPTION); request.setQuery(query);
     * 
     * IUserQueryResult secondResult = queryService.saveQuery(USER_ID, request);
     * 
     * // try to update the second query using // the same title in the first
     * query // thereby trying to create a duplicate query // for the same user,
     * which is not allowed request.setTitle(TITLE);
     * 
     * queryService.editQuery(USER_ID, request); }
     */
    ...
    private void deleteUserQuery(int id) {
        Query deleteQuery = entityManager
                .createQuery("delete from UserQuery where id = " + id);
