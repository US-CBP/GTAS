...
ENTRYPOINT cp -r /usr/share/logstash/config/* /logstash-conf/ && cp -r ./elasticsearch/config/* /elasticsearch-conf/ && echo y | ./elasticsearch/bin/elasticsearch-keystore create \
