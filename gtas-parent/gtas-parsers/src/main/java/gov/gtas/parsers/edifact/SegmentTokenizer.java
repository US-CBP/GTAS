...
public final class SegmentTokenizer {
	...
	public Segment buildSegment(String segmentText) throws ParseException {
		List<String> tokens = TextUtils.splitWithEscapeChar(segmentText, una.getDataElementSeparator(),
		...
		for (String cText : tokens) {
			List<String> elements = TextUtils.splitWithEscapeChar(cText, una.getComponentDataElementSeparator(),
					una.getReleaseCharacter());
