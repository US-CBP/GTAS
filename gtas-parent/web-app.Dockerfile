...
ENTRYPOINT mkdir -p /scheduler-logs/temp && cp -R /temp/* /usr/local/tomcat/conf/ && dockerize -wait tcp://${DB_HOST}:3306 -timeout 1000s logrotate /logrotate.conf && catalina.sh run