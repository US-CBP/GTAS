...
public class RuleConditionBuilderTest {
	...
	public void testSingleConditionApisSeat() throws ParseException {
		/*
		 * just one Seat condition. also test IN operator.
		 */
		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PASSENGER, PassengerMapping.SEAT,
	...
	public void testSingleConditionPnrSeat() throws ParseException {
		...
		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PNR, PassengerMapping.SEAT,
				CriteriaOperatorEnum.IN, new String[] { "A7865", "H76" }, TypeEnum.STRING);
		cond.setUuid(UUID.randomUUID());
		queryTerms.add(cond);
		setTestTargetUp(queryTerms);
	...
	public void testSingleConditionPassenger() throws ParseException {
		/*
		 * just one passenger condition. also test BETWEEN operator.
		 */

		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PASSENGER, PassengerMapping.DOB,
		...
		testTarget.buildConditionsAndApppend(result);
		assertTrue(result.length() > 0);
		// assertEquals("$p:Passenger("+PassengerMapping.DOB.getFieldName()+" >=
		// \"01-Jan-1990\", "
	...
	public void testSingleConditionFlight() throws ParseException {
		/*
		 * just one flight. also test IN operator.
		 */
		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.FLIGHT, FlightMapping.AIRPORT_DESTINATION,
	...
	public void testSeatAndFlight() throws ParseException {
		/*
		 * just one flight. also test ENDS_WITH operator.
		 */
		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.FLIGHT, FlightMapping.AIRPORT_DESTINATION,
				CriteriaOperatorEnum.IN, new String[] { "DBY", "XYZ", "PQR" }, TypeEnum.STRING);
		cond.setUuid(UUID.randomUUID());
		...
		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PASSENGER, PassengerMapping.SEAT,
	...
	public void testSeatAndDocument() throws ParseException {
		/*
		 * test just one document condition. also test BEGINS_WITH operator.
		 */
		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.DOCUMENT, DocumentMapping.ISSUANCE_COUNTRY,
				CriteriaOperatorEnum.NOT_EQUAL, "US", TypeEnum.STRING);

		cond.setUuid(UUID.randomUUID());
		queryTerms.add(cond);

		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PNR, PNRMapping.SEAT, CriteriaOperatorEnum.BEGINS_WITH,
				"29D", TypeEnum.STRING);
		cond.setUuid(UUID.randomUUID());
		queryTerms.add(cond);
		setTestTargetUp(queryTerms);
	...
	public void testSingleConditionDocument() throws ParseException {
		/*
		 * test just one document condition. also test NOT_EQUAL operator.
		 */
		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.DOCUMENT, DocumentMapping.ISSUANCE_COUNTRY,
				CriteriaOperatorEnum.NOT_EQUAL, "US", TypeEnum.STRING);
		cond.setUuid(UUID.randomUUID());
	...
	public void testStringConditionsOnPassenger() throws ParseException {
		for (CriteriaOperatorEnum op : new CriteriaOperatorEnum[] { CriteriaOperatorEnum.EQUAL,
				CriteriaOperatorEnum.NOT_EQUAL, CriteriaOperatorEnum.BEGINS_WITH, CriteriaOperatorEnum.ENDS_WITH,
				CriteriaOperatorEnum.CONTAINS, CriteriaOperatorEnum.NOT_BEGINS_WITH, CriteriaOperatorEnum.NOT_ENDS_WITH,
				CriteriaOperatorEnum.NOT_CONTAINS, CriteriaOperatorEnum.NOT_IN, CriteriaOperatorEnum.IN }) {
	...
	private void verifyStringConditionOnPassenger(CriteriaOperatorEnum op) throws ParseException {
		...
		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PASSENGER, PassengerMapping.LAST_NAME, op, val,
				TypeEnum.STRING);
		...
		case NOT_IN:
			...
			break;
		case BEGINS_WITH:
			assertEquals(prefix + attr + " != null, " + attr + " str[startsWith] \"" + val[0].toUpperCase() + "\")",
					result);
			break;
		case NOT_BEGINS_WITH:
			assertEquals(prefix + attr + " != null, " + attr + " not matches \"" + val[0].toUpperCase() + ".*\")",
					result);
			break;
		case ENDS_WITH:
			assertEquals(prefix + attr + " != null, " + attr + " str[endsWith] \"" + val[0].toUpperCase() + "\")",
					result);
			break;
		case NOT_ENDS_WITH:
			assertEquals(prefix + attr + " != null, " + attr + " not matches \".*" + val[0].toUpperCase() + "\")",
			...
			break;
		case CONTAINS:
			assertEquals(prefix + attr + " != null, " + attr + " matches \".*" + val[0].toUpperCase() + ".*\")",
					result);
			break;
		case NOT_CONTAINS:
			assertEquals(prefix + attr + " != null, " + attr + " not matches \".*" + val[0].toUpperCase() + ".*\")",
	...
	public void testDateConditionsOnPassenger() throws ParseException {
		for (CriteriaOperatorEnum op : new CriteriaOperatorEnum[] { CriteriaOperatorEnum.EQUAL,
				CriteriaOperatorEnum.NOT_EQUAL, CriteriaOperatorEnum.BETWEEN, CriteriaOperatorEnum.NOT_BETWEEN,
				CriteriaOperatorEnum.GREATER, CriteriaOperatorEnum.GREATER_OR_EQUAL, CriteriaOperatorEnum.LESS,
				CriteriaOperatorEnum.LESS_OR_EQUAL, CriteriaOperatorEnum.NOT_IN, CriteriaOperatorEnum.IN }) {
	...
	private void verifyDateConditionOnPassenger(CriteriaOperatorEnum op) throws ParseException {
		...
		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PASSENGER, PassengerMapping.DOB, op, val,
				TypeEnum.DATE);
		testTarget.addRuleCondition(cond);
		StringBuilder res = new StringBuilder();
		testTarget.buildConditionsAndApppend(res);
		...
		case LESS_OR_EQUAL:
			...
			break;
		case BETWEEN:
			assertEquals(prefix + attr + " >= \"" + formatRuleEngineDate(parseJsonDate(val[0])) + "\", " + attr
					+ " <= \"" + formatRuleEngineDate(parseJsonDate(val[1])) + "\")", result);
			break;
		case NOT_BETWEEN:
			assertEquals(prefix + "(" + attr + " < \"" + formatRuleEngineDate(parseJsonDate(val[0])) + "\" || " + attr
					+ " > \"" + formatRuleEngineDate(parseJsonDate(val[1])) + "\"))", result);
	...
	public void testMultipleConditionsDocument() throws ParseException {
		/*
		 * test multiple document conditions. also test GREATER_EQUAL and NOT_EQUAL.
		 */
		...
		queryTerms.add(cond);
		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.DOCUMENT, DocumentMapping.ISSUANCE_DATE,
	...
	public void testDocumentWithTypeEquality() throws ParseException {
		...
		cond.setUuid(docUUID);
		queryTerms.add(cond);
		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.DOCUMENT, DocumentMapping.DOCUMENT_TYPE,
	...
	public void testDocumentTypeEqualityOnly() throws ParseException {
		 ...
		 * one document condition and one type equality
		 */
		QueryTerm cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.DOCUMENT, DocumentMapping.DOCUMENT_TYPE,
				CriteriaOperatorEnum.EQUAL, "P", TypeEnum.STRING);
		cond.setUuid(UUID.randomUUID());
		queryTerms.add(cond);
		setTestTargetUp(queryTerms);
	...
	public void testMultipleConditionsPersonFlightDocument() throws ParseException {
		...
		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PASSENGER, PassengerMapping.DOB,
				CriteriaOperatorEnum.BETWEEN, new String[] { "1990-01-01", "1998-12-31" }, TypeEnum.DATE);
		testTarget.addRuleCondition(cond);
		...
		testTarget.addRuleCondition(cond);
		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.FLIGHT, FlightMapping.ETA, CriteriaOperatorEnum.EQUAL,
				"2015-01-01", TypeEnum.DATE);
		testTarget.addRuleCondition(cond);
		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.FLIGHT, FlightMapping.ETD, CriteriaOperatorEnum.EQUAL,
				"2015-01-01", TypeEnum.DATE);
		testTarget.addRuleCondition(cond);
		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.FLIGHT, FlightMapping.FLIGHT_NUMBER,
				CriteriaOperatorEnum.EQUAL, "2231", TypeEnum.INTEGER);
		testTarget.addRuleCondition(cond);

		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PASSENGER, PassengerMapping.LAST_NAME,
				CriteriaOperatorEnum.EQUAL, "Jones", TypeEnum.STRING);
		testTarget.addRuleCondition(cond);

		cond = RuleBuilderTestUtils.createQueryTerm(EntityEnum.PNR, PassengerMapping.SEAT, CriteriaOperatorEnum.EQUAL,
				new String[] { "A7865" }, TypeEnum.STRING);
		testTarget.addRuleCondition(cond);
		...
		assertEquals("$seat:Seat(" + RuleTemplateConstants.SEAT_ATTRIBUTE_NAME + " == \"A7865\", apis == false)\n"
