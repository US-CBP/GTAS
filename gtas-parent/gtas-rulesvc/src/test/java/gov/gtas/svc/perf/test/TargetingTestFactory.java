...
public class TargetingTestFactory implements PerformanceTestFactory, PerformanceTest {
	...
	public List<String> runTest() {
		...
		ret.add("Total Time = " + totalElapsed + ", Average Time = " + (totalElapsed / iterationCount));
		return ret;
	}

	@Override
	public PerformanceTest createTest(ConfigurableApplicationContext ctx) {
		targetingService = (TargetingService) ctx.getBean("targetingServiceImpl");
		watchlistService = (WatchlistService) ctx.getBean("watchlistServiceImpl");
	...
	private void genPerformanceData() {
		if (passengerWlCount > 0) {
			WatchlistRuleGenerator.generateWlRules(watchlistService, PASSENGER_WL_NAME, EntityEnum.PASSENGER,
					passengerWlCount);
		}
		if (documentWlCount > 0) {
			WatchlistRuleGenerator.generateWlRules(watchlistService, DOCUMENT_WL_NAME, EntityEnum.DOCUMENT,
					documentWlCount);
		}
		if (udrCount > 0) {
