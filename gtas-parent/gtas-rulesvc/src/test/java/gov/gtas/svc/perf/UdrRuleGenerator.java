...
public class UdrRuleGenerator {
	...
	public static final String[][][] UDR_DATA = { { { "Passenger", "firstName", "string", "BEGINS_WITH", "Jona" },
			{ "Passenger", "lastName", "string", "ENDS_WITH", "Smith" },
			{ "Passenger", "nationality", "string", "NOT_EQUAL", "USA" },
			{ "Flight", "carrier", "string", "EQUAL", "DL" }, { "Flight", "destination", "string", "EQUAL", "JFK" } },
			{ { "Passenger", "lastName", "string", "EQUAL", "Cooper" },
					...
					{ "Flight", "carrier", "string", "EQUAL", "DL" }, { "Flight", "origin", "string", "EQUAL", "LHR" },
					{ "Flight", "destination", "string", "EQUAL", "JFK" } },
			{ { "Passenger", "lastName", "string", "EQUAL", "Cooper" },
					{ "Document", "expirationDate", "date", "LESS", "2013-01-31" },
					{ "Address", "country", "string", "NOT_EQUAL", "USA" },
					{ "Pnr", "bagCount", "integer", "IS_NULL", "2" }, { "Flight", "carrier", "string", "EQUAL", "DL" },
	...
	private static UdrSpecification createUdr(String title, String[][] data, boolean invalidate) {
		...
		for (String[] term : data) {
			...
			bldr.addTerm(EntityEnum.getEnum(term[0]), term[1], TypeEnum.getEnum(term[2]),
					CriteriaOperatorEnum.getEnum(term[3]), new String[] { value });
		}
		return bldr.build();
	}
