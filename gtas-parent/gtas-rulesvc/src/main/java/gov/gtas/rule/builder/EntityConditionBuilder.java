...
public abstract class EntityConditionBuilder {
	/*
	 * list of criteria for the entity that will be combined to create an LHS term
	 * for the rule. Normally the criteria will be separated by ',' signifying an
	 * AND connector.
	 */
	 ...
	 * is used as connector. (an example is the NOT_BETWEEN operator, which contains
	 * a '||' connector.)
	 */
	...
	public void addCondition(final CriteriaOperatorEnum opCode, final String attributeName,
		...
		case EQUAL:
			if (attributeType == TypeEnum.DATETIME) {
				...
				RuleConditionBuilderHelper.addConditionValue(TypeEnum.DATE, values[0], bldr);
		...
		case LESS:
		case LESS_OR_EQUAL:
			bldr.append(attributeName).append(" ").append(opCode.getOperatorString()).append(" ");
		...
		case CONTAINS:
			addNotEmptyCondition(bldr, attributeName, ", ");
			bldr.append(attributeName).append(" ").append(opCode.getOperatorString()).append(" ");
			RuleConditionBuilderHelper.addConditionValue(attributeType, values[0], bldr, true, true);// begins and ends
																										// with
																										// wildcard
			break;
		case NOT_BEGINS_WITH:
			addNotEmptyCondition(bldr, attributeName, ", ");
			bldr.append(attributeName).append(" ").append(opCode.getOperatorString()).append(" ");
			RuleConditionBuilderHelper.addConditionValue(attributeType, values[0], bldr, false, true);// begins and ends
																										// with
																										// wildcard
			break;
		case NOT_ENDS_WITH:
			addNotEmptyCondition(bldr, attributeName, ", ");
			bldr.append(attributeName).append(" ").append(opCode.getOperatorString()).append(" ");
			RuleConditionBuilderHelper.addConditionValue(attributeType, values[0], bldr, true, false);// begins and ends
																										// with
																										// wildcard
			break;
		case NOT_CONTAINS:
	 ...
	 * CONTAINS require automatic insertion of the NOT_EMPTY condition, this method
	 * checks that such a condition has not been already added for this attribute.
	 * 
	 * @param bldr
	 * @param attributeName
	 */
	private void addNotEmptyCondition(StringBuilder bldr, String attributeName, String separatorString) {
		if (!this.attributesWithNotEmptyCondition.contains(attributeName)) {
			...
			bldr.append(attributeName).append(" ").append(CriteriaOperatorEnum.IS_NOT_EMPTY.getOperatorString());
			bldr.append(", ");
		}
	...
	/**
	 * @param generateEmptyClause
	 *            the generateEmptyClause to set
	 */
