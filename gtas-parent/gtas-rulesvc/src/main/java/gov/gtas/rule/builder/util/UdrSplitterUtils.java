...
public class UdrSplitterUtils {
	...
	public static List<List<QueryTerm>> createFlattenedList(QueryObject qobj) {
		...
		try {
			final QueryConditionEnum condOp = QueryConditionEnum.valueOf(qobj.getCondition());
			switch (condOp) {
			case OR:
				List<QueryEntity> termList = qobj.getRules();
				if (termList == null || termList.size() < 1) {
					throw ErrorHandlerFactory.getErrorHandler().createException(
							CommonErrorConstants.INVALID_ARGUMENT_ERROR_CODE, "rules (null or empty)",
							"JSON object details");
				}
			...
			case AND:
				...
				for (QueryEntity qtn : termList) {
					if (flatList.isEmpty()) {
						...
						flatList = multiplyFlatLists(flatList, createFlattenedList(qtn));
					}
				}
				break;
			default:
				throw ErrorHandlerFactory.getErrorHandler().createException(
						CommonErrorConstants.INVALID_ARGUMENT_ERROR_CODE, "condition:" + qobj.getCondition(),
						"JSON object details");
			}
	...
	private static List<List<QueryTerm>> multiplyFlatLists(List<List<QueryTerm>> list1, List<List<QueryTerm>> list2) {
		List<List<QueryTerm>> flatList = new LinkedList<List<QueryTerm>>();
