...
public class RuleConditionBuilderHelper {
	...
	private static String convertJsonStringVal(final TypeEnum type, final String value,
			final boolean beginsWithWildcard, final boolean endsWithWildcard) throws ParseException {
		String ret = null;
		switch (type) {
		case BOOLEAN:
			if (value.charAt(0) == 'Y') {
	...
	/**
	 * Creates a regular expression that begins/ends or both begins and ends with
	 * wild cards.
	 * 
	 * @param value
	 *            the string to put in the regex.
	 * @param beginsWithWildcard
	 *            if true a wildcard is inserted at the start of the regex.
	 * @param endsWithWildcard
	 *            if true a wildcard is appended to the regex.
	 * @return
	 */
	private static String createRegex(String value, final boolean beginsWithWildcard, final boolean endsWithWildcard) {
	...
	public static void addConditionValue(final TypeEnum type, final String val, final StringBuilder bldr,
			final boolean beginsWithWildcard, final boolean endsWithWildcard) throws ParseException {
	...
	public static void addConditionValues(final TypeEnum type, final String[] values, StringBuilder bldr)
			throws ParseException {
		bldr.append(LEFT_PAREN_CHAR);
		if (values != null && values.length > 0) {
			bldr.append(convertJsonStringVal(type, values[0], false, false));
			for (int i = 1; i < values.length; ++i) {
				bldr.append(COMMA_CHAR).append(SPACE_CHAR).append(convertJsonStringVal(type, values[i], false, false));
			}
		}
	...
	public static void addConditionDescription(final QueryTerm cond, StringBuilder bldr) throws ParseException {
		...
		bldr.append(entity.getEntityName()).append(SPACE_CHAR).append(getFieldName(entity, cond.getField()))
				.append(SPACE_CHAR).append(opCode.getDisplayName()).append(SPACE_CHAR);
