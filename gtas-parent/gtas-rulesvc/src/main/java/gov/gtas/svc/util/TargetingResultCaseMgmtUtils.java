...
public class TargetingResultCaseMgmtUtils {
	...
	private static final Logger logger = LoggerFactory.getLogger(TargetingResultCaseMgmtUtils.class);
	 ...
	 * @return
	 */
	public static Set<Case> ruleResultPostProcesssing(List<RuleHitDetail> resultList,
			CaseDispositionService dispositionService, PassengerService passengerService) {
		...
		Map<Long, List<Long>> passengerFlightMap = TargetingResultUtils.createPassengerFlightMap(resultList,
				passengerService);
			...
			if (rhd.getFlightId() == null) {
				...
				if (flightIdList != null && !CollectionUtils.isEmpty(flightIdList)) {
					for (Long flightId : flightIdList) {
						Case caze = processPassengerFlight(rhd, flightId, caseMap, flightMap, passengerMap, ruleCatMap,
								dispositionService);
				...
				Case caze = processPassengerFlight(rhd, rhd.getFlightId(), caseMap, flightMap, passengerMap, ruleCatMap,
						dispositionService);
				if (!casesSet.add(caze)) {
					casesSet.remove(caze);
					casesSet.add(caze);
	 ...
	 */
	private static Case processPassengerFlight(RuleHitDetail rhd, Long flightId, Map<Long, Case> caseMap,
		...
		try {
			...
			if (_tempPax != null) {
				...
				if (_tempPax.getDocuments() != null) {
					document = _tempPax.getDocuments().stream().findFirst().map(Document::getDocumentNumber)
							.orElse(null);
				}
				newCase = dispositionService.registerCaseFromRuleService(flightId, rhd.getPassengerId(),
						rhd.getPassengerName(), rhd.getPassengerType().getPassengerTypeName(),
						_tempPax.getPassengerDetails().getNationality(), _tempPax.getPassengerDetails().getDob(),
						document, description, rhd.getRuleId(), caseMap, flightMap, passengerMap, ruleCatMap);
			}
		} catch (Exception ex) {
			logger.error("Could not initiate a case for Flight:" + flightId + "  Pax:" + _tempPaxId + "  Rule:"
					+ rhd.getRuleId() + " set", ex);
		}
	...
	private static Map<Long, Case> createCaseMap(List<RuleHitDetail> ruleHitDetailList,
			CaseDispositionService dispositionService) {
