...
(function() {
  "use strict";
  app.controller("CaseDispositionCtrl", function(
    $scope,
    $rootScope,
    $http,
    $mdToast,
    ...
    $scope.enableEmailNotification = configService
      .enableEmailNotificationService()
      .then(function(value) {
    ...
    $scope.hitTypeIcon = function(hitType) {
      ...
      if (hitType) {
        ...
        if (hitType.includes("P")) {
          icons +=
            '<i class="fa fa-user" style="color:rgb(255, 176, 22)"></i>&nbsp;';
        }
    ...
    $scope.resolvePage = function() {
      ...
      caseDispositionService.getPagedCases(postData).then(function(data) {
    ...
    $scope.refreshCountDown = function() {
      ...
      for (var i = 0; i < $scope.casesList.length; i++) {
        ...
        if (
          $scope.casesList[i].flightDirection === "O" ||
          $scope.casesList[i].flightDirection === "C"
        ) {
    ...
    var fixGridData = function(grid, row, col, value) {
      ...
      if (col.name === "highPriorityRuleCatId") {
        value =
          grid.appScope.casesListWithCats[row.entity.highPriorityRuleCatId];
      }
      return value;
    };
    ...
    $scope.deleteRow = function(row) {
      ...
      caseDispositionService
        .updatePassengerHitViews(row.entity, "REVIEWED")
    ...
    $scope.getPaxDetailReport = function(row) {
      ...
      paxReportService
        .getPaxDetailReport(pax.paxId, pax.flightId)
        .then(function(data) {
          if (data) {
            ...
            consol.log(
              "ERROR! Error in generating GTAS Event Report. No data was retured"
            );
          }
        });
    ...
    $scope.reOpen = function(row) {
      ...
      caseDispositionService
        .updatePassengerHitViews(row.entity, "RE_OPENED")
        .then(function(result) {});
    ...
    $scope.casesDispGrid.columnDefs = [
      ...
      },
      {
        ...
        cellTemplate:
          '<div style="font-family: \'Roboto Mono\', monospace"><md-button aria-label="type" ng-click="grid.appScope.showPassenger(row)" ' +
    ...
    $scope.filterCheck = function(option) {
      var filters = [
        ...
        "onlyMyRules",
        "ruleTypes"
      ]; //, 'priority', 'dateLabel', 'date'
      return filters.includes(option);
    };

    $scope.clearFilters = function() {
      let defaultSort = [{ column: "countDown", dir: "asc" }];
      ...
      $scope.model.pageSize =
        typeof $scope.model.pageSize != "undefined"
          ? $scope.model.pageSize
          : 10;
      $scope.model.origin = [];
      $scope.model.dest = [];
    ...
    $scope.filter = function() {
      spinnerService.show("html5spinner");
      var toastPosition = angular.element(
        document.getElementById("casesDispGrid")
      );
      caseDispositionService
        .getByQueryParams($scope.model)
        .then(function(data) {
          if (data != null && data.data.cases != null) {
            if (data.data.cases.length < 1) {
              $scope.errorToast(
                $translate.instant("msg.noresultsfound"),
                toastPosition
              );
            }
            $scope.casesDispGrid.data = data.data.cases;
