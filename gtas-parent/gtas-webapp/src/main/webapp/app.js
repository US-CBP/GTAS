...
(function() {
  ...
  var pageDefaults = {
    ...
    initialize = function(
      ...
      $rootScope.currentlyLoggedInUser = $sessionStorage.get(
        APP_CONSTANTS.CURRENT_USER
      );

      if (
        $rootScope.currentlyLoggedInUser === undefined ||
        $rootScope.currentlyLoggedInUser == null
      ) {
      ...
      $rootScope.searchBarQuery = function() {
        if (
          $rootScope.currentLocation.val != "/adhocquery" &&
          typeof $rootScope.searchBarContent != "undefined" &&
          $rootScope.searchBarContent.content != "" &&
          $rootScope.searchBarContent.content.length > 1
        ) {
    ...
    router = function(
      ...
      $stateProvider
        ...
        .state("dash", {
          // temporary mapping to show sample dashboard data
          url: "/dash",
          roles: [
            USER_ROLES.ADMIN,
            USER_ROLES.VIEW_FLIGHT_PASSENGERS,
            USER_ROLES.MANAGE_QUERIES,
            USER_ROLES.MANAGE_RULES,
            USER_ROLES.MANAGE_WATCHLIST
          ],
          authenticate: true,
          views: {
            "@": {
        ...
        .state("flights", {
          ...
          roles: [
            USER_ROLES.ADMIN,
            USER_ROLES.VIEW_FLIGHT_PASSENGERS,
            USER_ROLES.MANAGE_CASES,
            USER_ROLES.MANAGE_QUERIES,
            USER_ROLES.MANAGE_RULES,
            USER_ROLES.MANAGE_WATCHLIST,
            USER_ROLES.ONE_DAY_LOOKOUT
          ],
          authenticate: true,
          views: {
            "@": {
        ...
        .state("casemanual", {
          ...
          resolve: {
            passenger: function(paxDetailService, $stateParams) {
              return paxDetailService.getPaxDetail(
                $stateParams.paxId,
                $stateParams.flightId
              );
            },
        ...
        .state("casedetail", {
          ...
          resolve: {
            newCases: function(caseDispositionService, $stateParams) {
              return caseDispositionService.getOneHitsDisposition(
                $stateParams.caseId
              );
            }
          }
        ...
        .state("onedaylookout", {
          ...
          resolve: {
            lookoutData: function(oneDayLookoutService, $stateParams) {
              return oneDayLookoutService.getOneDayLookout(
                $stateParams.flightDate
              );
            }
          }
        })
        ...
        .state("adhocquery", {
          ...
          resolve: {
            searchBarResults: function(adhocQueryService, $rootScope) {
              ...
              if (
                typeof $rootScope.searchBarContent != "undefined" &&
                $rootScope.searchBarContent.content.length > 0
              ) {
                return adhocQueryService.getPassengers(
                  $rootScope.searchBarContent.content,
                  1,
                  10,
                  defaultSort
        ...
        .state("flightpax", {
          ...
          resolve: {
            ...
            passengers: function(paxService, $stateParams, paxModel) {
              return paxService.getPax(
                $stateParams.id,
                paxModel.alldatamodel()
              );
            },
            user: function(userService) {
              return userService.getUserData();
        ...
        .state("detail", {
          ...
          resolve: {
            ...
            ruleHits: function(paxService, $stateParams) {
              return paxService.getRuleHitsByFlightAndPax(
                $stateParams.paxId,
                $stateParams.flightId
              );
            },
    ...
    NavCtrl = function(
      ...
      $scope.onRoute = function(key) {
        return (
          (lookup[key].name &&
            lookup[key].name.indexOf($scope.stateName) >= 0) ||
          (lookup[key].mode && lookup[key].mode.indexOf($scope.mode) >= 0)
        );
      };
      $scope.showNav = function() {
        return (
          ["queryFlights", "queryPassengers", "detail"].indexOf(
            $scope.stateName
          ) === -1
        );
      };
      notificationService.getErrorData().then(
        function(value) {
  ...
  app = angular
    ...
    .constant("APP_CONSTANTS", {
      ...
      LOGIN_ERROR_MAX_ATTEMPTS:
        "Too many failed attempts to log in, please check your email"
    })
    .run(initialize)
      ...
      return {
        checkRole: function(user) {
          ...
          currentUser.hasRole = function(requiredRole) {
            ...
            for (var i = 0; i < currentUser.roles.length; i++) {
              if (
                String(currentUser.roles[i].roleDescription).toLowerCase() ===
    ...
    .directive("myElem", function() {
      return {
        ...
        link: function(scope, element, attrs) {} //end watch
      };
    })
      ...
      function(timeUtil, $interval) {
        return {
          ...
          link: function(scope, element, attrs) {
            ...
            var messageWheelsUp =
              '&nbsp;<span class="label label-danger">WheelsUp</span>';
              ...
              $interval(function() {
                ...
                if (Math.floor((future - current) / 86400000) < 2) {
                  if (scope.showcountdownlabelflag === "true") {
                    ...
                    message =
                      messageStart + timeUtil.dhms(diff) + messageEndDiv;
                  }
                } else {
                  message = messageStart + timeUtil.dhms(diff) + messageEndDiv;
                }
