...
var droolsOut = function (queryBuilder) {
    return prepareDroolsJSON(queryBuilder.getRules());
};

var droolsIn = function (queryBuilder) {
    return interpretDroolsJSON( JSON.parse(localStorage.getItem(queryName)) );
};
...
QueryBuilder.prototype.loadRules = function(queryName) {
    ...
    if (queryName === 'Empty / New') {
        this.reset();
        return;
    }

    if (!rules) {
        console.log('query invalid, empty, or deleted / nonexistent');
        return;
    }
...
//    rules = JSON.stringify(this.adapter(service, 'out')
    ...
    if( this.settings.service !== undefined ) rules = JSON.stringify( this.adapter('out') );
