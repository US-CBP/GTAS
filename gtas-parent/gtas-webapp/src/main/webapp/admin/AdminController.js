...
app.controller("AdminCtrl", function(
  ...
  $scope.codeColList[CARRIER] = [
    { name: "iata", len: 2 },
    { name: "name", len: 255 }
  ];
  $scope.codeColList[AIRPORT] = [
    { name: "iata", len: 3 },
    { name: "icao", len: 4 },
    { name: "name", len: 255 },
    { name: "city", len: 255 },
  ...
  $scope.codeColList[COUNTRY] = [
    { name: "iso2", len: 2 },
    { name: "iso3", len: 3 },
    { name: "name", len: 255 },
    { name: "isoNumeric", len: 3 }
  ];
  ...
  var setupUserGrid = function() {
    ...
    $scope.userGrid.columnDefs = gridOptionsLookupService.getLookupColumnDefs(
      "admin"
    );
  };
  var setupAuditGrid = function() {
    $scope.auditGrid = gridOptionsLookupService.getGridOptions("audit");
    $scope.auditGrid.columnDefs = gridOptionsLookupService.getLookupColumnDefs(
      "audit"
    );
  };
  var setupErrorGrid = function() {
    $scope.errorGrid = gridOptionsLookupService.getGridOptions("error");
    $scope.errorGrid.columnDefs = gridOptionsLookupService.getLookupColumnDefs(
      "error"
    );
  };
  var setupZipGrid = function() {
    ...
    $scope.zipGrid.columnDefs = gridOptionsLookupService.getLookupColumnDefs(
      ZIPLOGS
    );
  };
  ...
  var setupCodeGrids = function() {
    ...
    $scope.carrierGrid.columnDefs = gridOptionsLookupService.getLookupColumnDefs(
      CARRIER
    );

    $scope.countryGrid = Object.assign({}, gridTemplate);
    $scope.countryGrid.columnDefs = gridOptionsLookupService.getLookupColumnDefs(
      COUNTRY
    );

    $scope.airportGrid = Object.assign({}, gridTemplate);
    $scope.airportGrid.columnDefs = gridOptionsLookupService.getLookupColumnDefs(
      AIRPORT
    );
  };
  ...
  var refreshZipData = function() {
    fileDownloadService
      .getLogZipList($scope.activeZipType)
      .then(bindZipGrid, $scope.errorToast);
  };
  ...
  $scope.refreshActiveCodeGrid = function() {
    ...
    codeService
      .getAllCodes($scope.activeCodeTab)
      .then(setCodeData, $scope.errorToast);
  };
  ...
  var selectErrorRow = function(gridApi) {
    ...
    $scope.errorGridApi.selection.on.rowSelectionChanged($scope, function(row) {
  ...
  $scope.viewCodes = function(tab) {
    ...
    $scope.toastParent = $document[0].getElementById(
      $scope.activeCodeTab + "Grid"
    );
    $scope.refreshActiveCodeGrid();
  };
  ...
  $scope.saveCode = function() {
    ...
    if (!isEqual) {
      ...
      if (action === "edit") {
        codeService
          .updateCode(tab, $scope.rowSelected)
          .then(function(response) {
            if (response.status === "FAILURE") {
              ...
              $scope.refreshActiveCodeGrid();
              that.successToast(
                $translate.instant("msg.codesuccessfullysaved")
              );
              $scope.OK = false;
            }
          });
        ...
        codeService
          .createCode(tab, $scope.rowSelected)
  ...
  $scope.downloadZip = function(row) {
    if (!!row && !!$scope.activeZipType)
      fileDownloadService.getLogZip($scope.activeZipType, row.fileName);
  };
  ...
  $scope.deleteCode = function() {
    ...
    if (r == true) {
      codeService
        .deleteCode(tab, $scope.rowSelected.id)
        .then($scope.refreshActiveCodeGrid, $scope.errorToast);
      refreshTooltips();
  ...
  $scope.restoreCode = function() {
    ...
    if (r == true) {
      codeService
        .restoreCode(tab, $scope.rowSelected)
        .then($scope.refreshActiveCodeGrid, $scope.errorToast);
      refreshTooltips();
  ...
  $scope.restoreAllCodes = function() {
    ...
    if (r == true) {
      ...
      if ($scope.OK) {
        that.successToast(
          $translate.instant(`msg.${tab}codessuccessfullyrestored`)
        );
        $scope.OK = false;
      }
    }
  ...
  $scope.lastSelectedUser = function(user) {
    localStorage["lastSelectedUser"] = JSON.stringify(user);
  };
  ...
  $scope.refreshAudit = function() {
    ...
    auditService
      .getAuditData(
        model.action,
        model.user,
        model.timestampStart,
        model.timestampEnd
      )
      .then(setAuditData, $scope.errorToast);
  };
  $scope.refreshError = function() {
    var model = $scope.errorModel;
    $scope.selectedErrorItem = null;
    errorService
      .getErrorData(model.code, model.timestampStart, model.timestampEnd)
      .then(setErrorData, $scope.errorToast);
  };
  ...
  $scope.saveOrEditStatus = function() {
    caseService
      .createOrEditDispositionStatus($scope.createDispStatusVo())
      .then(function(response) {
  ...
  $scope.deleteStatus = function() {
    caseService
      .deleteDispositionStatus($scope.createDispStatusVo())
      .then(function(response) {
        //produce success message
        if (response.status === 200) {
          $scope.errorToast(response.data.message);
      ...
      .error(function(response) {
        $scope.errorToast(
          response.data.statusText +
            " " +
            $translate.instant("msg.maynotremovestatus")
        );
      });
  };
  ...
  $scope.saveSettings = function() {
    defaultSettingsService.saveSettings($scope.settingsInfo).then(
