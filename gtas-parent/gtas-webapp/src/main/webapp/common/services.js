...
(function() {
  ...
  app
    ...
    .service("errorService", function($http, $q) {
      ...
      return {
        getErrorData: function(code, commence, fin) {
          var st =
            commence != null ? moment(commence).format("YYYY-MM-DD") : null;
      ...
      return {
        ...
        getCountryTooltips: function() {
          if (localStorage.getItem("countriesList") !== null) {
            ...
            return Promise.resolve(
              response.map(x => ({ id: x.iso3, name: getTidyName(x.name) }))
            );
          } else {
        ...
        getCarrierTooltips: function() {
          if (localStorage.getItem("carriersList") !== null) {
            return Promise.resolve(
              JSON.parse(localStorage.getItem("carriersList"))
            );
          } else {
      ...
      var today = moment().format("YYYY-MM-DD"),
        pageOfPages = function(currentPage, pageCount) {
          return (
            ...
            (pageCount === 1
              ? ""
              : "\t" + currentPage.toString() + " of " + pageCount.toString())
          );
        },
        standardOptions = {
          ...
          airport: [
            {
              ...
              width: "1%",
              cellTemplate:
                '<a class="full-width editLink" ng-click="grid.appScope.openSidebarEdit(row.entity)"><i class="fa fa-edit"></a>'
            },
            {
              name: "id",
              field: "id",
          ...
          ziplogs: [
            {
              ...
              width: "2%",
              cellTemplate:
                '<a class="full-width editLink" ng-click="grid.appScope.downloadZip(row.entity)"><i class="fa fa-download"></a>'
            },
            {
              name: "fileName",
            ...
            },
            {
              ...
              field: "size",
              cellTemplate:
                "<div>{{grid.appScope.formatBytes(COL_FIELD)}}</div>",
              width: "15%"
            },
            {
              name: "creationDate",
              ...
              cellTemplate:
                "<div>{{COL_FIELD | date:'yyyy-MM-dd HH:mm:ss'}}</div>",
              field: "creationDate",
              width: "20%"
            },
            {
    ...
    .service("gridService", function() {
      ...
      var calculateGridHeight = function(numRows) {
        ...
        anyWatchlistHit = function(grid, row) {
          if (
            row.entity.onWatchList ||
            row.entity.onWatchListDoc ||
            row.entity.onWatchListLink
          ) {
    ...
    .service("passengersBasedOnUserFilter", function(
      userService,
      flightService,
      flightsModel,
      $q
          ...
          return userService
            ...
            .then(function(user) {
              if (user.data.filter != null) {
                ...
                if (
                  typeof user.data.filter.etaStart != undefined &&
                  user.data.filter.etaStart != null
                ) {
                  flightsModel.starteeDate = new Date();
                  flightsModel.etaStart.setDate(
                    today.getDate() + user.data.filter.etaStart
                  );
                }
                if (
                  typeof user.data.filter.etaEnd != undefined &&
                  user.data.filter.etaEnd != null
                ) {
                  flightsModel.endDate = new Date();
                  flightsModel.etaEnd.setDate(
                    today.getDate() + user.data.filter.etaEnd
                  );
                } // Response Handler #1
                ...
                if (user.data.filter.destinationAirports != null)
                  flightsModel.destinations =
                    user.data.filter.destinationAirports;
              }
              return flightsModel;
            });
        },
