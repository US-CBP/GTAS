...
app.controller("BuildController", function(
  ...
  var todayDate = moment().toDate(),
    ...
    queryPassengersLink = document.querySelector(
      'a[href="#/query/passengers"]'
    ),
    conditions,
    model = {
      summary: {
    ...
    loadOnSelection = {
      ...
      rule: function(row) {
        $scope.selectedIndex = $scope.qbGrid.data.indexOf(row.entity);
        jqueryQueryBuilderService
          .loadRuleById("rule", row.entity.id)
  ...
  $scope.setData = {
    ...
    rule: function(myData) {
      var temp,
        data = [];
      myData.forEach(function(obj) {
        var startDateObj = $scope.convertUTCDateToLocalDate(
          obj.summary.startDate
        );
        obj.summary.startDate = startDateObj.toISOString().substring(0, 10);
        if (obj.summary.endDate != null) {
          var endDateObj = $scope.convertUTCDateToLocalDate(
            obj.summary.endDate
          );
  ...
  $scope.getToolbarText = function() {
    switch ($scope.buttonMode) {
      case "query":
        return $scope.mode === "query" && $scope.ruleId !== null
          ? $translate.instant("qry.updatequery")
          : $translate.instant("qry.savequery");
      case "rule":
        return $scope.mode === "rule" &&
          $scope.ruleId !== null && !$scope.isCopy
          ? $translate.instant("qry.updaterule")
          : $translate.instant("qry.saverule");
    }
  };
  ...
  $scope.executeQuery = function(e) {
    var m = moment(),
      timestamp =
        m.day() + "|" + m.hours() + "|" + m.minutes() + "|" + m.seconds(),
    ...
    localStorage["qbTitle"] = $scope[$scope.mode].title.length
      ? $scope[$scope.mode].title.trim()
      : "";
  ...
  $scope.delete = function() {
    var selectedRowEntities = $scope.gridApi.selection
        .getSelectedRows()
        .reverse(),
      rowIndexToDelete;
  ...
  $scope.save = {
    query: {
      confirm: function() {
        ...
        spinnerService.show("html5spinner");
        jqueryQueryBuilderService
          .save("query", queryObject)
          .then($scope.updateQueryBuilderOnSave);
      }
    },
    rule: {
      confirm: function() {
        ...
        if ($scope.ruleId === null || $scope.isCopy) {
          if (ruleObject.summary.startDate.getDate() < new Date().getDate()) {
            $scope.openAlert(
              "Invalid Start Date",
              "Start date must be today or later when created new"
            );
            $scope.saving = false;
            return;
          }
        ...
        if (ruleObject.summary.endDate) {
          if (ruleObject.summary.endDate < ruleObject.summary.startDate) {
            $scope.openAlert(
              "Invalid End Date",
              "End Date must be empty/open or be >= Start Date"
            );
            $scope.saving = false;
            return;
          }
  ...
  $scope.fixInClauses = function(ruleArray) {
    for (var j = 0; j < ruleArray.rules.length; j++) {
      if (
        ruleArray.rules[j].operator == "IN" &&
        ruleArray.rules[j].value.length == 1
      ) {
  ...
  $(document).on("mouseenter", "a[id*='bs-select']", function() {
    $(this).prop(
      "title",
      codeTooltipService.getCodeTooltipData($(this).text(), "dictionary")
    );
  });
  ...
  $(document).on(
    "mouseenter",
    "select[name='builder_rule_0_value_0']",
    function() {
      $(this)
        .find("option")
        .each(function() {
