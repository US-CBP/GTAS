...
public class ParseDataSegment {
	 ...
	 * @return java.lang.String value of the APIS field extracted from the data
	 *         segment
	 *
	 */
	...
	public static String extractDateFieldData(String dataSegment, String delimiter, String currentFormat,
	 ...
	 * @param delimiter
	 *            the delimiter required to parse the data segment
	 * @return java.lang.String the arrival or departure date as string
	 *
	 */

	public static String extractFlightDateField(String flightDataSegment, String comDataElementSep)
		...
		if (flightDataSegment != null && !flightDataSegment.isEmpty()) {
			...
			if (firstIndex != -1 && firstIndex + 7 <= flightDataSegment.length()) {
				...
				if (flightDateTimeStr.trim().length() == 6)
					flightDateTime = ApisStringParser.parseDateTimeAsStr(flightDateTimeStr,
							Constants.DATE_FORMAT_YYMMDD, Constants.DATE_FORMAT_YYYY_MM_DD);
			}
	 ...
	 * @param delimiter
	 *            the delimiter required to parse the data segment
	 * @return java.lang.String the arrival or departure date time as string
	 *
	 */

	public static String extractFlightDateTimeField(String flightDataSegment, String comDataElementSep)
			throws ParseException {
		...
		if (flightDataSegment != null && !flightDataSegment.isEmpty()) {
			...
			if (firstIndex != -1 && lastIndex != -1 && firstIndex != lastIndex) {
				...
				if (timePeriodCode != null && timePeriodCode.trim().equals(Constants.YYMMDDHHMM_CODE)) {
					...
					flightDateTime = ApisStringParser.parseDateTimeAsStr(flightDateTimeStr,
							Constants.DATE_FORMAT_YYMMDDHHMM, Constants.DATE_FORMAT_YYYY_MM_DD_HH_MM);
				}
	 ...
	 * @param delimiter
	 *            the delimiter required to parse the data segment
	 * @return java.lang.String value of the APIS field extracted from the middle of
	 *         the data segment
	 * 
	 ...
	 * 
	 * @return java.lang.String array A string array of size 3 as follows [0] =
	 *         Telephone number, null if the data is not present [1] = Fax number,
	 *         null if the data is not present [2] = Email address, null if the data
	 *         is not present
	 *
	 */
	public static String[] getContactInfo(String communicationSegment, String dataElementSeparator,
			String componentDataElementSep) {
		String[] contactInfo = new String[3];

		if (communicationSegment != null && !communicationSegment.isEmpty()
				&& communicationSegment.trim().startsWith("COM")) {
				...
				for (int i = 0; i < comSegArray.length; i++) {
					if (comSegArray[i] != null) {
						if (comSegArray[i].contains("TE")) {
							contactInfo[0] = ApisStringParser.getSubstrFistToDelimiter(comSegArray[i],
									componentDataElementSep);
						} else if (comSegArray[i].contains("FX")) {
							contactInfo[1] = ApisStringParser.getSubstrFistToDelimiter(comSegArray[i],
									componentDataElementSep);
						} else if (comSegArray[i].contains("EM")) {
							contactInfo[2] = ApisStringParser.getSubstrFistToDelimiter(comSegArray[i],
									componentDataElementSep);
						}
					}
				}
