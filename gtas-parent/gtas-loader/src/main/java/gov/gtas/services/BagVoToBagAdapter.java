...
class BagVoToBagAdapter {
    ...
    BagVoToBagAdapter(MessageVo messageVo,
                      Set<Bag> passengerBags,
                      Set<BookingDetail> messageBookingDetails) {
    ...
    private Map<UUID, UUID> getOrphanMap(MessageVo messageVo, Set<BookingDetail> messageBookingDetails) {
        ...
        if (messageBookingDetails.size() < messageVo.getFlights().size() - 1) {
            ...
            Set<UUID> bookingDetails = messageBookingDetails.stream().map(BookingDetail::getParserUUID).collect(Collectors.toSet());
