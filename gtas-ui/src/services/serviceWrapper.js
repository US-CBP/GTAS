...
function setOps(ctx, ...fxns) {
  ...
  const context = (isObject(ctx)) ? {uri: ctx.uri, contentType: ctx.contentType || JSON} : {uri: ctx, contentType: JSON}
...
//  CRUD OPERATIONS
  function post(body) {
    ...
    // console.log(body);
    return GenericService({uri: this.uri, method: POST, contentType: this.contentType, body: body});
  }
  ...
  function put(body, id) {
    ...
    const query = `\\${id}`;
    return GenericService({uri: this.uri+query, method: PUT, contentType: this.contentType, body: body});
  }
  ...
  export const flights = setOps(FLIGHTS, get);
